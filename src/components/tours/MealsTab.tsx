...
export function MealsTab({ tourId, meals, onChange, tour, readOnly = false }: MealsTabProps) {
  ...
  } = useMealsTab({ tourId, meals, onChange, tour, readOnly });
  ...
  return (
    <div className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-3">
              <div className="flex gap-2">
                        ...
                        <CommandGroup>
                          {detailedExpenses.map((item) => (
                            <CommandItem key={item.id} value={item.name} onSelect={() => {
                              setFormData({ ...formData, name: item.name, price: item.price, date: formData.date || new Date().toISOString().split('T')[0] });
                              setOpenMeal(false);
                            }}>
                ...
                <Button type="button" variant="outline" size="icon" onClick={() => setShowNewMealDialog(true)} title="Thêm bữa ăn mới">
              ...
              }} min={0} max={tour?.totalGuests || 0} placeholder="Số khách" className="w-full" />
            </div>
      ...
      <NewMealDialog
        ...
        readOnly={readOnly} onSubmit={handleCreateNewMeal} onCreateCategory={handleCreateNewCategory}
      />
    </div>
  );
}
